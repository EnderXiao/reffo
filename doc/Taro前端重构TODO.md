...
- [-] 将所有业务页面从 `resumeStore`、`jdStore` 兼容映射迁移到 `resumeWorkspaceStore`。
- [x] 首页、创建页、结果页已改为通过 workspace selector/action 访问业务状态，移除页面级 workspace `getState()` 读取。
- [-] 删除创建页和结果页中剩余的 `getState()` 直接读取，改为模型层统一 selector 或 command；关键业务读写已完成，兼容层读取仍待收口。
- [ ] 明确 workspace Store 状态机：`idle`、`analyzing`、`matching`、`optimizing`、`interviewing`、`completed`、`failed`。
- [ ] 禁止同一业务状态同时存在于页面 local state、旧 Store、workspace Store 三处。
- [ ] 为 workspace Store 补充状态迁移、重置、并发请求和取消请求测试。
- [ ] 完成兼容层下线评估，确认没有页面、测试和文档依赖后删除 `resumeStore/jdStore`。

### P0：请求层收敛

- [-] 保留 `ApiClient` 对外 API，降低 `TaroRequestAdapter` 为内部 transport。
- [ ] 将请求发送、错误归一化、重试拆为职责清晰的内部函数或模块。
- [ ] 删除 adapter 对外暴露而业务不需要的 interceptor 扩展面，除非确有平台适配场景。
- [ ] 统一 HTTP 错误、业务错误、取消请求和超时错误语义。
- [ ] 为 GET/POST/上传/取消/重试分别补最小契约测试。
- [ ] 检查 `src/services/README.md`，删除与当前实现不一致的旧 API 示例。

### P0：路由复用落地

- [ ] 将 `src/utils/navigation.ts` 限定为 Taro transport 和兼容逻辑，业务页面不直接依赖其细节。
- [ ] 完成 Landing、首页、创建、结果、完成、认证、个人资料页面的路径常量迁移。
- [ ] 统一页面参数类型，避免页面内重复解析 `router.params`。
- [ ] 为 `useRouteTransition` 增加错误回退、重复点击、页面栈过深和 H5 深链接测试。
- [ ] 清理生产代码中剩余的硬编码 `/pages/...` 路径。

### P0：动画复用与边界

- [ ] 将创建页生成态切换中的通用 View Transition 调用迁移到 `runViewTransition`。
- [ ] 将可复用的 timeout/RAF 清理迁移到 `useAnimationLifecycle`，优先处理创建页、结果页和 Landing 页。
- [ ] 保留业务专属动画在业务组件内：HomeCardDeck FLIP、结果页卡片回流、删除简历破裂动画不做万能配置化。
- [ ] 将 `navigation-transition.ts` 拆分为：路由 CSS/transition、业务卡片回流、transition suppression 三个边界。
- [ ] 统一 `prefers-reduced-motion`、H5 fallback 和 transition cleanup 行为。
...
- [ ] 删除 H5 aliases/mocks 中不再需要的模块。
- [ ] 最终从 `package.json` 移除不再使用的 RN/Expo 依赖，并同步 lockfile。

### P1：组件复用治理

- [ ] 按职责划分 `common`、`business`、页面私有 `components`，禁止跨页面复制同类组件。
- [ ] 抽取稳定的展示组件和交互模型，不抽取只复用一次的业务 JSX。
- [ ] 为上传区、岗位描述输入、生成阶段、页面头部、反馈操作建立明确 props 契约。
...
- [ ] 补公共路由参数、`useRouteTransition`、`usePageStateRoute` 测试。
- [ ] 补 workspace Store 与 session manager 集成测试。
- [ ] 修复全量 Jest 中剩余旧页面契约失败，目标为全绿。
- [ ] 建立 H5 构建、Jest、`git diff --check` 的最小 CI 门禁。
- [ ] 单独记录全量 `tsc` 遗留类型问题，区分平台边界问题和本次改动回归。

### P1：构建与性能

- [ ] 生成 H5 bundle 分析报告，确认大体积来源。
...
- [ ] 生产代码只有一个业务 workspace 状态源。
- [ ] 页面不直接拼接路由路径，不直接处理底层请求细节。
- [ ] 通用路由、页面状态路由、计时器、RAF、View Transition 均有公共入口。
- [ ] 业务专属动画保持业务边界，不被过度抽象。
- [ ] H5 构建无本次改动引入的错误。
- [ ] Jest 全量通过，关键页面拥有状态、路由、异常和卸载测试。
- [ ] RN/Expo 残留有明确清理结果，依赖和 lockfile 一致。
- [ ] 文档、脚本、测试和目录结构反映 H5-only 架构。

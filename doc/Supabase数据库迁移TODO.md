...
| --- | --- | --- | --- | --- |
| 非生产环境 | 日常开发联调、自动化测试、上线前回归，可按需重置 | `reffo-nonprod` | 测试/预发后端部署或本地 `.env.test` | 可使用脱敏样本和临时数据，允许清库重建；不能直接复用正式用户数据。 |
...
| --- | --- | --- | --- | --- | --- |
| SB-0 | blocked | P0 | 确认 Supabase 环境划分 | Supabase project | 明确非生产、正式两套 Supabase project 或数据库实例命名、负责人和用途；正式环境与非生产环境物理隔离。 |
| SB-1 | blocked | P0 | 收集两套环境项目信息 | SB-0 | 分别拿到非生产、正式的 `SUPABASE_URL`、`SUPABASE_PUBLISHABLE_KEY`、`SUPABASE_SECRET_KEY`，并确认 Secret key 只保存于对应后端环境或本地安全环境。 |
...
| SB-7 | blocked | P0 | 在非生产环境执行并验证 schema | SB-4, SB-5, SB-6 | migration 在非生产 Supabase 成功执行；测试用户 A 无法读取用户 B 数据。 |
| SB-8 | blocked | P0 | 非生产环境主链路回归 | SB-7 | 非生产环境完成登录、保存源简历、生成简历、查看历史、删除历史 smoke，再允许准备正式执行。 |
| SB-9 | todo | P0 | 正式环境 migration 执行预案 | SB-8 | 正式执行前有备份、执行窗口、回滚或前滚修复预案；不直接用未验证 SQL 操作正式库。 |
...
| SB-20 | todo | P1 | 增加 SQLite 到 Supabase 迁移脚本 | SB-13, SB-14 | 新增 `backend/scripts/migrate-sqlite-to-supabase.ts`，可从 `backend/data/reffo.sqlite` 导入到指定 `MIGRATION_USER_ID`。 |
| SB-21 | done | P0 | 补后端单测和集成测试 | SB-15, SB-18, SB-19 | SQLite provider 现有测试通过；Supabase repository 至少覆盖字段映射、用户过滤、JSONB 读写和错误路径。 |
| SB-22 | todo | P0 | 前端接入 Supabase Auth 登录态 | SB-16 | 前端可登录并拿到 access token；核心业务请求继续走后端 API。 |
| SB-23 | todo | P0 | 前端 API 请求注入 Bearer token | SB-22 | `source-resume`、生成流程、历史相关请求都能携带 token；无登录态时有明确处理。 |
| SB-24 | todo | P0 | 多用户隔离端到端验证 | SB-18, SB-19, SB-23 | 用户 A 和用户 B 分别创建源简历和历史后，互相无法通过接口读取、更新或删除。 |
| SB-25 | todo | P1 | 准备非生产和正式部署配置 | SB-10, SB-15, SB-17 | 非生产、正式后端分别注入对应 Supabase project 环境变量；正式环境设置 `APP_ENV=prod` 和 `DATABASE_PROVIDER=supabase`。 |
| SB-26 | todo | P1 | 上线前 smoke 验证 | SB-24, SB-25 | 非生产环境先完成主链路 smoke；正式 migration 后完成生产 smoke，并记录结果。 |
| SB-27 | todo | P2 | 迁移文件上传到 Supabase Storage | SB-22 | PDF / JD 图片写入各环境私有 bucket；后端按用户权限读取文件并调用 OCR；删除策略明确。 |
| SB-28 | todo | P2 | Harness 拆到独立 SQLite 运行时库 | 无 | Harness 表不再混用主业务 SQLite；支持 `HARNESS_DATABASE_PATH`、保留天数和最大 run 数。 |
| SB-29 | todo | P2 | 增加数据库备份、回滚和清理说明 | SB-9, SB-25 | 文档说明非生产和正式各自备份策略、migration 回滚或前滚修复方案、SQLite 导入脚本回滚方式。 |

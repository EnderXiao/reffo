...
import { buildInterviewAdviceMessages, buildInterviewBusinessRepairMessages, buildJsonRepairMessages } from '@/prompts/prompts'
...
export class InterviewAdvisorAgent {
  ...
  async repairBusinessOutput(
    ...
    try {
      const response = await this.provider.complete({
        messages: buildInterviewBusinessRepairMessages({
          analysis,
          matching,
          optimizedResume,
          currentOutput,
        ...
        responseFormat: 'json_object',
        temperature: 0,
        promptVersion: getPromptVersion('interview-advisor.business-repair', promptVariant),
        eventBus: options.eventBus,
        stepContext: options.stepContext,
      })
      ...
      const parsedOutput = await parseJsonOutput({
        ...
        repair: async ({ content, errorMessage, outputName }) => {
          const repairResponse = await this.provider.complete({
            ...
            responseFormat: 'json_object',
            temperature: 0,
            promptVersion: getPromptVersion('interview-advisor.business-repair.json', promptVariant),
            eventBus: options.eventBus,
            stepContext: options.stepContext,
          })
      ...
      console.error('Interview advice business repair failed:', error)
      throw new Error(`面试建议业务修复失败: ${error instanceof Error ? error.message : '未知错误'}`)
    }
  }
}

...
import { buildJsonRepairMessages, buildMatchingBusinessRepairMessages, buildMatchingMessages } from '@/prompts/prompts'
...
export class MatchingAgent {
  ...
  async repairBusinessOutput(
    ...
    try {
      const response = await this.provider.complete({
        messages: buildMatchingBusinessRepairMessages({
          resume,
          jd,
          currentOutput,
          evaluation,
        ...
        responseFormat: 'json_object',
        temperature: 0,
        promptVersion: getPromptVersion('matching.business-repair', promptVariant),
        eventBus: options.eventBus,
        stepContext: options.stepContext,
      })
      ...
      const parsedOutput = await parseJsonOutput({
        ...
        repair: async ({ content, errorMessage, outputName }) => {
          const repairResponse = await this.provider.complete({
            ...
            responseFormat: 'json_object',
            temperature: 0,
            promptVersion: getPromptVersion('matching.business-repair.json', promptVariant),
            eventBus: options.eventBus,
            stepContext: options.stepContext,
          })
      ...
      throw new Error(`匹配分析业务修复失败: ${error instanceof Error ? error.message : '未知错误'}`)
    }
  }
}

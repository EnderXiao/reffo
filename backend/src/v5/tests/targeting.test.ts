...
describe('job-targeted internal contracts', () => {
  ...
  test.each(['jd_as_evidence', 'missing_proof', 'missing_difference', 'duplicate_target', 'unsupported_narrative'] as const)('rejects concrete mapping error %s', mutation => {
    const fixture = createTargetingFixture()
    ...
    if (mutation === 'unsupported_narrative') fixture.fit.narratives[0].evidenceIds = [fixture.skill.evidenceId]
  ...
  test.each(['fake_target', 'fake_evidence', 'unlinked_evidence', 'jd_evidence'] as const)('weak narrative still rejects an invalid reference: %s', mutation => {
    const fixture = createTargetingFixture()
  ...
  test('Writer receives task links and only selected facts, not an extra stage', () => {
    ...
    expect(payload.facts.every(fact => !fact.evidenceId.startsWith('job:'))).toBe(true)
